...
const said = (text: string) => [{ role: "assistant", stopReason: "stop", content: [{ type: "text", text }] }];
assert.equal(shouldFire(base, said("Draft above. **OK to continue?**")), false, "pause line: no fire");
assert.equal(shouldFire(base, said("Done. OK to continue? I also fixed X.")), true, "pause line not at end: fires");
assert.equal(shouldFire(base, said("Want me to do more?")), true, "other questions: fires");
